...
from bestman.core.config import BESTMAN_HOME, load_config, get_current_stage, load_env, save_plan
...
class Voyage:
    ...
    def render_map(self, today_advance=0, sway_offset=0.0, sway_phase=0.0) -> str:
        ...
        """
        render_data = self.map_engine.build_render_data(
            self.state.get_tiles_revealed(),
            today_advance=today_advance,
            sway_offset=sway_offset,
            sway_phase=sway_phase,
        )
        from bestman.renderers.ascii import AsciiRenderer
        return AsciiRenderer().render_map(render_data, self.theme, None)
