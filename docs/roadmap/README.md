...
| `recap` 长期/周/月/自动回顾 | 已落地，有 LLM/template fallback |
...
- `recap --auto` 按完成天数在 7/30 天节点生成周/月回顾。
- 同一天同周期已生成过回顾时跳过，避免重复日志。
- 仍然写 `RecapGenerated` 事件，可通过 SQLite projection 重建。
- 这是显式 CLI 检查，不引入后台调度服务。
